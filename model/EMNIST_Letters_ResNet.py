...
from constant import get_image_test, get_emnist_letter_class, get_device, emnist_transformer
import os
DEVICE = get_device()
...
class ResNet18(nn.Module):  # 构建resnet18层
    ...
    def __init__(self):
        ...
        self.linearLayer1 = nn.Linear(256, 27)  # 最后是全连接层
...
def model_predict(img, model):
    if isinstance(model, ResNet18):
        ...
        return get_emnist_letter_class(int(pred.data))
    return -1
...
def get_model_files():
    ...
    model_resnet = os.path.join(model_dir, "model_file/model_emnist_letters_resnet.ckpt")
    return [model_resnet]